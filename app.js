...
const routes = [
  {
    title: 'Old Town art hour',
    time: '60 to 90 minutes',
    description: 'A compact route for visitors who want a strong art hit without crossing the city.',
    stops: ['Fruitmarket', 'Stills', 'Dovecot Studios'],
    tags: ['central', 'walking', 'tourist friendly']
  },
  {
    title: 'Calton Hill slow route',
    time: 'Half day',
    description: 'A quieter route built around views, galleries and enough breathing room to actually look properly.',
    stops: ['Collective', 'Calton Hill', 'Old Town cafe stop'],
    tags: ['quiet', 'views', 'free']
  },
  {
    title: 'Festival survival route',
    time: 'Flexible',
    description: 'Designed for August: fewer tabs, clearer choices, and a walkable route instead of an overwhelming list.',
    stops: ['Start near Waverley', 'Choose two major venues', 'Add one artist-run space'],
    tags: ['festival', 'easy plan', 'low stress']
  }
];
...
function formatDate(item) {
  ...
  return {
    ...
    meta: ongoing ? `Ongoing until ${longDate.format(end)}` : `From ${longDate.format(start)}`
  };
}
...
function renderListings() {
  ...
  todayCount.textContent = `${activeListings().length} open listings`;
